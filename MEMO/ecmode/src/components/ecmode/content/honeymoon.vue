  ...
  <transition name="el-fade-in-linear">
    ...
    <div class="panel" style="padding:30px">
      ...
      <el-button-group>
        ...
        <el-button type="primary" plain @click="addVisible = true">添加</el-button>
      </el-button-group>
      ...
      <el-table ref="multipleTable" :data="list" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="text.title" label="标题"></el-table-column>
        <el-table-column prop="text.label" label="标签"></el-table-column>
        <el-table-column prop="text.subTitle" label="子标题"></el-table-column>
        <el-table-column prop="text.content" label="描述"></el-table-column>
        <el-table-column  label="图片">
          <template slot-scope="scope">
            <img v-bind:src="ip+scope.row.text.image1" height="50px">
            <img v-bind:src="ip+scope.row.text.image2" height="50px" v-if="scope.row.text.image2 != ''">
          </template>
        </el-table-column>
      ...
      <el-dialog title="新增" :visible.sync="addVisible"  center width="60%">
        <span>标题</span><br>
        <el-input  v-model="addData.text.title"></el-input>
        <span>标签</span><br>
        <el-input  v-model="addData.text.label"></el-input>
        <span>子标题</span><br>
        <el-input  v-model="addData.text.subTitle"></el-input>
        <el-upload class="upload-demo" :data="{label:'image1'}"  drag action="http://tzhop.free.ngrok.cc/" multiple :http-request="fileUpload">
          <i v-if="addData.text.image1 == ''" class="el-icon-upload"></i>
            <img v-if="addData.text.image1 != ''" v-bind:src="ip+addData.text.image1" width="100%">
            <div v-if="addData.text.image1 == ''" class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
        </el-upload>
        <el-upload class="upload-demo" :data="{label:'image2'}"  drag action="http://tzhop.free.ngrok.cc/" multiple :http-request="fileUpload">
          <i v-if="addData.text.image2 == ''" class="el-icon-upload"></i>
            <img v-if="addData.text.image2 != ''" v-bind:src="ip+addData.text.image2" width="100%">
            <div v-if="addData.text.image2 == ''" class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
        </el-upload>

        <span>定制师描述</span><br>
        <el-input type="textarea" v-model="addData.text.content"></el-input>
      ...
      <el-dialog title="修改" :visible.sync="updateVisible"  center width="60%">
        <span>标题</span><br>
        <el-input  v-model="addData.text.title"></el-input>
        <span>标签</span><br>
        <el-input  v-model="addData.text.label"></el-input>
...
export default {
  ...
  methods:{
    ...
    update(){
      this.loading = true;
      var that = this;
      that.addData.text = JSON.stringify(that.addData.text);
      ...
      that.addData.text = JSON.parse(that.addData.text);
    },
    submit(){
      this.loading = true;
      var that = this;
    ...
    fileUpload(e){
      ...
      $.ajax({
        ...
        success:function (res){
          res = JSON.parse(res);
          if(res.code == 1000){
            that.addData.text[e.data.label] = res.body;
            that.$message({
                type: 'success',
                message: res.result
            });
