  ...
  <transition name="el-fade-in-linear">
    ...
    <div class="panel" style="padding:30px">
      ...
      <el-button-group>
        ...
        <el-button type="primary" plain @click="addVisible = true">添加图片</el-button>
      </el-button-group>
      ...
      <el-dialog title="新增图片" :visible.sync="addVisible"  center width="30%">
      ...
      <el-dialog title="修改图片" :visible.sync="updateVisible"  center width="30%">
        <el-upload class="upload-demo" :data="{label:'image'}"  drag action="http://tzhop.free.ngrok.cc/" multiple :http-request="fileUpload">
          <i v-if="addData.image == ''" class="el-icon-upload"></i>
            <img v-if="addData.image != ''" v-bind:src="ip+addData.image" width="100%">
            <div v-if="addData.image == ''" class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
...
export default {
  data(){
    return {
      ...
      multipleSelection:[],
      pageOptions:[
        {label:'首页',value:'/me_home'},
        {label:'定制旅行',value:'/costomer_order_tour'},
        {label:'定制师',value:'/me_tailor'},
        {label:'主题游',value:'/me_theme_traval'},
        {label:'案例故事',value:'/me_story_comment'},
        {label:'蜜月旅行',value:'/me_honeymoon'},
        {label:'关于我们',value:'/me_aboutus'},
        {label:'旅行攻略',value:'/me_toursim'},
        {label:'旅行知识',value:'/me_knowlege'},
        {label:'新闻资讯',value:'/me_news'},
        {label:'加入我们',value:'/me_joinus'}
      ],
      pageModel:{
        page:1,
        size:10,
