    ...
    <div class="lpanel">
      <div class="panell">
        ...
        <el-button-group style="margin-top:20px;float:right">
          <el-button type="info" @click="navToRegister">首页</el-button>
          <el-button type="primary" :loading="loginLoading" @click="doLogin()">登录</el-button>
        </el-button-group>
        <div style="clear:both"></div>
      </div>
      <div class="team">sunwou network tec company<br>sunwou.com</div>
    </div>
  </div>
</transition>
</template>
...
export default {
  ...
  mounted(){
    $('.login-panel').css("height",window.innerHeight);
     $('.lpanel').css("height",window.innerHeight);
  },
  methods: {
    doLogin(){
      ...
      if(this.adminName == 'admin'){
        if(this.adminPwd == 'admin'){
          this.$store.state.user = 1;
          that.$router.push({path:'/ec_main'});
        }else{
