<template>
  <div>
    <div class="animated fadeInUp panel">
      ...
      <div>
        <span style="color:#888888">位用户更好的体验，每件商品最多放5张图片。</span>
      </div>
      <hr>
      <div style="padding-bottom:15px">
        <el-button :plain="true" type="danger" icon="delete" @click="multiDelete()">批量删除</el-button>
        <a href="javascript:;" style="width:120px;height:40px;overflow:hidden;display:inline-block; background:#ff9900;line-height:40px; text-align:center;color:#fff;border-radius:5px">
          <span>添加图片</span>
          <input style="margin-top:-40px;opacity:0;width:120px;height:40px;" type="file" id="imagePath" @change="fileUpload('imagePath')">
        </a>
      ...
      <el-table ref="multipleTable" :data="list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="图片">
          <template scope="scope">
            <div style="padding:5px 0"><img v-bind:src="ip+scope.row.imagePath" height="150px"></div>
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" >
          <template scope="scope">
            ...
            <el-button type="text" size="small" @click="deleteSimpleRow(scope.row.carouselId)">删除</el-button>
      ...
      <el-form :label-position="'left'" :model="company" :rules="companyRules" ref="company" label-width="0" class="demo-ruleForm">
        <el-row :gutter="30">
          <el-col :span="10">
            ...
            <input type="file" id="imagePaths" @change="fileUploads('imagePaths')">
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
  ...
  methods: {
      ...
      $.ajax({
        ...
        success:function (res){
          if(res.code == 1000){
            ...
            $.post(that.$store.state.yip+'carousel/info/add',{
              commodityId:that.$route.params.id,
              imagePath:that.company.imagePath
            },function(res){
              that.getCategory();
            })
          }else{
    ...
    fileUploads(name){
      ...
      $.ajax({
        ...
        success:function (res){
          if(res.code == 1000){
            that.company.imagePath = res.result;
            that.$message({
                type: 'success',
                message: '上传成功'
            });
    ...
    submitCompany: function (e) {
      var that = this;
      var intfa = 'add';
      if(that.company.carouselId){
        intfa = 'update';
      }
      $.post(this.$store.state.yip+'carousel/info/update',this.company,function(res){
        if(res.code == 1000){
            var msg = '规格添加成功';
