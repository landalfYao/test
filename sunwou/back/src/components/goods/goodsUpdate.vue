<template>
  <div style="padding:20px;background:#fff;border-radius:15px">
    <el-form :model="commodity"   label-width="100px" class="demo-ruleForm">
      <el-form-item label="所属分类" required v-if="this.$store.state.user.commodityType == 1">
      ...
      <el-form-item label="所属分类" required v-if="this.$store.state.user.commodityType == 2">
        <el-select v-model="commodity.categoryTwoId" placeholder="二级类目" v-if="categoryTwo.length > 0">
          <el-option v-for="item in categoryTwo" :key="item.categoryTwoId" :label="item.categoryTwoName" :value="item.categoryTwoId">
          </el-option>
        </el-select>
      ...
      <el-form-item>
        <el-button type="primary" id="submitCreate" :loading="isCreate">确认更新</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    editorUpload: function() {
      ...
      editor.create();
      $.post(that.$store.state.yip + 'commodity/info/find',{
      ...
      $("#submitCreate").click(function(){
        ...
        $.post(that.$store.state.yip + 'commodity/info/update',commodity,function (res) {
          that.isCreate = false;
          if(res.code == 1000){
            that.$message({
                type: 'success',
