      ...
      <el-table ref="multipleTable" :data="list" border tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column fixed="right" label="操作" >
          <template scope="scope">
            ...
            <el-button type="text" size="small" @click="deleteSimpleRow(scope.row.categoryId)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-form :label-position="'left'" :model="company" :rules="companyRules" ref="company" label-width="0" class="demo-ruleForm">
        <el-row :gutter="30">
          <el-col :span="8">
            ...
            <el-form-item>
              <el-input v-model="company.categoryName" placeholder="分类名称"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <span style="line-height:30px">
  ...
  methods: {
    ...
    initMultiSelection: function () {
      ...
      for(var item in this.multipleSelection){
        list.push(this.multipleSelection[item].categoryId);
      }
      return list;
    },
    //取消选择
    ...
    multiDelete: function () {
      ...
      this.$confirm('此操作将批量删除该信息, 是否继续?', '提示', {
          ...
          $.get(that.$store.state.yip+'resources/category/deletes?categoryIds='+that.initMultiSelection(),function(res){
            if(res.code == 1000){
              that.$message({
                type: 'success',
                message: res.result
    ...
    deleteSimpleRow: function (id){
      ...
      this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
          ...
          $.get(that.$store.state.yip+'resources/category/deletes?categoryIds='+id,function(res){
            if(res.code == 1000){
              that.$message({
                type: 'success',
                message: res.result
    ...
    submitCompany: function (e) {
      var that = this;
      var intfa = 'add';
      if(that.company.categoryId){
        intfa = 'update';
      }
      $.post(this.$store.state.yip+'resources/category/'+intfa,this.company,function(res){
        if(res.code == 1000){
            var msg = '分类信息添加成功';
            if(that.company.categoryId){
              msg = '分类信息修改成功';
