  ...
  <div style="padding:20px;background:#fff;border-radius:15px">
    <el-form :model="commodity"   label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="资源视频路径填写" >
        <el-input type="text" v-model="commodity.imagePath" placeholder="http://"></el-input>
      </el-form-item>
      <el-form-item label="资源视频" required>
        <a class="photoUpload" href="javascript:;">
      ...
      <el-form-item>
        <el-button type="primary" @click="submitsd()">立即修改</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  mounted() {
    ...
    $.ajax({
      url:this.$store.state.yip+'resources/info/find',
      data:{resourcesId:this.$route.params.id},
      dataType:'JSON',
      method:'POST',
  ...
  methods: {
    submitsd(){
      var that = this;
      $.post(that.$store.state.yip + 'resources/info/update',this.commodity,function (res) {
          that.isCreate = false;
          if(res.code == 1000){
            that.$message({
                type: 'success',
            ...
            that.$router.push({name:'vedioManage'})
          }else{
            that.$message({
                type: 'warring',
                message: res.result
