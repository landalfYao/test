...
<transition name="el-fade-in-linear">
  <div>
        <div class="card pd-20 pd-sm-40">
          <h6 class="card-body-title">添加商品</h6>
            ...
            <div class="row mg-b-25">
              <div class="col-lg-12">
                <div class="form-group">
                  ...
                  <el-cascader :options="category" @active-item-change="categoryChange" :props="props" ></el-cascader>
                </div>
              </div><!-- col-4 -->
    ...
    submitAttr(){
      ...
      $.ajax({
        ...
        success(res){
          console.log(res)
          if(res.code == 1000){
            that.attr.push({id:res.result,name:that.attrData.name,image:that.attrData.image,price:that.attrData.price,weight:that.attrData.weight});
    ...
    editorUpload: function() {
      ...
      $("#submitCreate").click(function() {
        ...
        for(var i in that.attr){
          goods.attributes += that.attr[i].id + ',';
        }
        $.ajax({
          type: "POST",
          url: that.$store.state.ip + "commodityadd",
          data: goods,
          dataType:'JSON',
          success(res){
            if (res.code == 1000) {
